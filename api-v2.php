...
if (isset($_POST['access_key']) && isset($_POST['delete_user_account'])) {
    ...
    if (isset($_POST['user_id'])) {
        ...
        $sql = 'DELETE FROM `daily_leaderboard` WHERE `user_id`=' . $id;
        $db->sql($sql);

        $sql1 = 'DELETE FROM `monthly_leaderboard` WHERE `user_id`=' . $id;
        $db->sql($sql1);

        $sql2 = 'DELETE FROM `users_statistics` WHERE `user_id`=' . $id;
        $db->sql($sql2);

        $sql3 = 'DELETE FROM `battle_statistics` WHERE `user_id1`=' . $id;
        $db->sql($sql3);

        $sql4 = 'DELETE FROM `battle_statistics` WHERE `user_id2`=' . $id;
        $db->sql($sql4);

        $sql5 = 'DELETE FROM `tbl_level` WHERE `user_id`=' . $id;
        $db->sql($sql5);

        $sql6 = 'DELETE FROM `contest_leaderboard` WHERE `user_id`=' . $id;
        $db->sql($sql6);

        $sql7 = 'DELETE FROM `daily_quiz_user` WHERE `user_id`=' . $id;
        $db->sql($sql7);

        $sql8 = 'DELETE FROM `question_reports` WHERE `user_id`=' . $id;
        $db->sql($sql8);

        $sql9 = 'DELETE FROM `tbl_bookmark` WHERE `user_id`=' . $id;
        $db->sql($sql9);

        $sql10 = 'DELETE FROM `tbl_bookmark` WHERE `user_id`=' . $id;
        $db->sql($sql10);

        $sql11 = 'DELETE FROM `tbl_tracker` WHERE `user_id`=' . $id;
        $db->sql($sql11);

        $sql12 = 'DELETE FROM `users` WHERE `id`=' . $id;
        $db->sql($sql12);
