...
if (isset($_GET['table']) && $_GET['table'] == 'category') {
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $where .= " AND ( c.`id` like '%" . $search . "%' OR c.`category_name` like '%" . $search . "%' OR l.`language` like '%" . $search . "%' OR c.`type` like '%". $type."%' )";
    }
...
//banner_image
if(isset($_GET['table']) && $_GET['table']=='banner_image'){
     $offset = 0;
    $limit = 10;
    $sort = 'id';
    $order = 'ASC';
    ...
    $sql = "SELECT COUNT(s.id) as total FROM `banner_image` s ";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT s.* FROM `banner_image` s " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    ...
    $i = 1;
    foreach ($res as $row) {
        $image = (!empty($row['image'])) ? 'images/banner_image/' . $row['image'] : '';
...
// 2.1 sub_2category
if (isset($_GET['table']) && $_GET['table'] == 'sub_2category') {
    $offset = 0;
    $limit = 10;
    $sort = 'row_order';
    $order = 'ASC';
    ...
    if (isset($_GET['type']) && !empty($_GET['type'])) {
        ...
        if ($type == 2 || $type == '2') {
            $total_question = ", (SELECT count(id) FROM tbl_maths_question WHERE tbl_maths_question.sub_2category = s.id ) as no_of_que";
        }
    }
    ...
    if (isset($_GET['language']) && !empty($_GET['language'])) {
        ...
        if (isset($_GET['subcategory']) && !empty($_GET['subcategory'])) {
            $where .= ' AND `subcategory_id`=' . $_GET['subcategory'];
        }
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $where .= " AND (s.`id` like '%" . $search . "%' OR s.`subcategory_id` like '%" . $search . "%' OR s.`sub_2category_name` like '%" . $search . "%' OR l.`language` like '%" . $search . "%' OR sub.`subcategory_name` like '%" . $search . "%' )";
    }

    $left_join = " LEFT JOIN languages l on l.id = s.language_id ";
    $left_join .= " LEFT JOIN subcategory sub ON sub.id = s.subcategory_id ";
    $left_join .= " LEFT JOIN category c ON c.id = sub.maincat_id ";
    

    $sql = "SELECT COUNT(s.id) as total FROM `sub_2category` s " . $left_join . " " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT s.*, l.language, c.`category_name`,sub.`subcategory_name`  FROM `sub_2category` s " . $left_join . " " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    ...
    $i = 1;
    foreach ($res as $row) {
        $image = (!empty($row['image'])) ? 'images/sub_2category/' . $row['image'] : '';
        $operate = "<a class='btn btn-xs btn-primary edit-subcategory' data-id='" . $row['id'] . "' data-toggle='modal' data-target='#editCategoryModal' title='Edit'><i class='fas fa-edit'></i></a>";
        $operate .= "<a class='btn btn-xs btn-danger delete-subcategory' data-id='" . $row['id'] . "' data-image='" . $image . "' title='Delete'><i class='fas fa-trash'></i></a>";
        ...
        $tempRow['category_name'] = $row['category_name'];
        $tempRow['subcategory_name'] = $row['subcategory_name'];
        $tempRow['sub_2cat_name']=$row['sub_2category_name'];
        $tempRow['row_order'] = $row['row_order'];
        $tempRow['image'] = (!empty($row['image'])) ? '<a href="' . $image . '" data-lightbox="Sub Category Images"><img src="' . $image . '" height=30 ></a>' : '<img src="images/logo-half.png" height=30>';
        $tempRow['status'] = ($row['status']) ? '<label class="label label-success">Active</label>' : '<label class="label label-danger">Deactive</label>';
        $tempRow['no_of_que'] = $row['no_of_que'];
...
//get ebook
if (isset($_GET['table']) && $_GET['table'] == 'ebook') {
    $offset = 0;
    $limit = 10;
    $sort = 'id';
    $order = 'ASC';
    ...
    if (isset($_GET['language']) && !empty($_GET['language'])) {
        $where .= ' AND s.`language_id` = ' . $_GET['language'];
        if (isset($_GET['category']) && !empty($_GET['category'])) {
            $where .= ' AND s.`category_id`=' . $_GET['category'];
            if (isset($_GET['subcategory']) && !empty($_GET['subcategory'])) {
                $where .= ' AND s.`subcategory_id`=' . $_GET['subcategory'];
            }
            if(isset($_GET['sub_2category']) && !empty($_GET['sub_2category'])){
            $where .=' AND s.`sub_2category_id`='.$_GET['sub_2category'];
        }
        }
    } elseif (isset($_GET['category']) && !empty($_GET['category'])) {
        $where .= ' AND s.`category_id` = ' . $_GET['category'];
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        // $where .= " AND (s.`id` like '%" . $search . "%' OR sub.`subcategory_name` like '%" . $search . "%' OR sub_2.`sub_2category_name` like '%" . $search . "%' OR l.`language` like '%" . $search . "%' OR c.`category_name` like '%" . $search . "%' )";
        $where .=" AND (s.id like '%".$search."%' OR sub.subcategory_name like'%".$search."%' OR sub_2.sub_2category_name like '%".$search."%' OR l.language like '%".$search."%' OR c.category_name like '%".$search."%')";
    }

    $left_join = " LEFT JOIN languages l on l.id = s.language_id ";
    $left_join .= " LEFT JOIN subcategory sub ON sub.id = s.subcategory_id ";
    $left_join .= " LEFT JOIN category c ON c.id = s.category_id ";
    $left_join .="LEFT JOIN sub_2category sub_2 ON sub_2.id=s.sub_2category_id";
    

    $sql = "SELECT COUNT(s.id) as total FROM `ebook` s " . $left_join . " " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT s.*, l.language, c.`category_name`,sub.`subcategory_name`,sub_2.sub_2category_name  FROM `ebook` s " . $left_join . " " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
...
//dailynews
if (isset($_GET['table']) && $_GET['table'] == 'dailynews') {
    $offset = 0;
    $limit = 10;
    $sort = 'id';
    $order = 'ASC';
    ...
    $sql = "SELECT COUNT(s.id) as total FROM `dailynews` s " . $left_join . " " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    ...
    $sql = "SELECT s.*, l.language, c.`category_name` " . $total_question . " FROM `dailynews` s " . $left_join . " " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    ...
    foreach ($res as $row) {
        // $image = (!empty($row['image'])) ? 'images/subcategory/' . $row['image'] : '';
         $operate = "<a class='btn btn-xs btn-primary edit-category' href='edit-daily-news.php?id=".$row['id']."'  title='Edit'><i class='fas fa-edit'></i></a>";
        $operate .= "<a class='btn btn-xs btn-danger delete-subcategory' data-id='" . $row['id'] . "' title='Delete'><i class='fas fa-trash'></i></a>";
        $tempRow['id'] = $row['id'];
        $tempRow['srno']=$i;
        $tempRow['language_id'] = $row['language_id'];
        $tempRow['language'] = $row['language'];
        ...
        $tempRow['category_name'] = $row['category_name'];
        $tempRow['title'] = $row['title'];
        $tempRow['detail'] = $row['description'];
...
//end
//currentaffairs
if (isset($_GET['table']) && $_GET['table'] == 'currentaffair') {
    $offset = 0;
    $limit = 10;
    $sort = 'id';
    $order = 'ASC';
    ...
    if (isset($_GET['language']) && !empty($_GET['language'])) {
        $where .= ' AND s.`language_id` = ' . $_GET['language'];
        if (isset($_GET['category']) && !empty($_GET['category'])) {
            $where .= ' AND s.`maincat_id`=' . $_GET['category'];
            if(isset($_GET['subcategory']) && !empty($_GET['subcategory'])){
                $where .=' AND s.subcategory_id='.$_GET['subcategory'];
            }
        }
    ...
    $sql = "SELECT s.*, l.language, c.`category_name`,sub.`subcategory_name` " . $total_question . " FROM `currentaffair` s " . $left_join . " " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
...
$i = 1;
    foreach ($res as $row) {
        // $image = (!empty($row['image'])) ? 'images/subcategory/' . $row['image'] : '';
        $operate = "<a class='btn btn-xs btn-primary edit-category' href='edit-currentaffair.php?id=".$row['id']."' title='Edit'><i class='fas fa-edit'></i></a>";
        $operate .= "<a class='btn btn-xs btn-danger delete-subcategory' data-id='" . $row['id'] . "'  title='Delete'><i class='fas fa-trash'></i></a>";
...
if (isset($_GET['type']) && !empty($_GET['type'])) {
    ...
    if (isset($_GET['language']) && !empty($_GET['language'])) {
        ...
        if (isset($_GET['category']) && !empty($_GET['category'])) {
            ...
            if(isset($_GET['sub_2category']) && !empty($_GET['sub_2category'])){
            $where .=' AND `sub_2category_id`='.$_GET['sub_2category'];
        }
        }
    } elseif (isset($_GET['category']) && !empty($_GET['category'])) {
        $where .= ' AND `category` = ' . $_GET['category'];
