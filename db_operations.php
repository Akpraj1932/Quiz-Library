...
// 30 get_subcategories_of_categories
if (isset($_POST['get_subcategories_of_categories']) && $_POST['get_subcategories_of_categories'] != '') {
    $id = $_POST['maincat_id'];
    $type = $_POST['type'];
    if (empty($id)) {
        echo '<option value="">Select Sub Category</option>';
    ...
    $sql = 'SELECT * FROM `subcategory` WHERE `maincat_id`=' . $id . ' AND `type`=' . $type . ' ORDER BY id + 0 ASC';
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_POST['get_subcategories_of_language']) && $_POST['get_subcategories_of_language'] != '') {
    $id = $_POST['language_id'];
    ...
    $sql = 'SELECT * FROM `subcategory` WHERE `language_id`=' . $id . ' AND `type`=' . $type . ' AND maincat_id='.$cat_id.' ORDER BY row_order + 0 ASC';
    $db->sql($sql);
    $res = $db->getResult();
    ...
    if (isset($_POST['sortable']) && $_POST['sortable'] == 'sortable') {
        ...
        foreach ($res as $category) {
            ...
            if (!empty($category["image"])) {
                $options .= "<li id='" . $category["id"] . "'><big>" . $category["row_order"] . ".</big> &nbsp;<img src='images/category/$category[image]' height=30 > " . $category["subcategory_name"] . "</li>";
            } else {
                $options .= "<li id='" . $category["id"] . "'><big>" . $category["row_order"] . ".</big> &nbsp;<img src='images/logo-half.png' height=30 > " . $category["subcategory_name"] . "</li>";
            }
        }
...
//end
//get_sub_2categories_of_language
if (isset($_POST['get_sub_2categories_of_language']) && $_POST['get_sub_2categories_of_language'] != '') {
    $id = $_POST['language_id'];
    $cat_id = $_POST['category'];
    // $type = (isset($_POST['type'])) ? $_POST['type'] : 1;
    $type = $_POST['type'];
    if (empty($id)) {
        echo '<option value="">Select Sub 2 Category</option>';
        return false;
    }
    $sql = 'SELECT * FROM `sub_2category` WHERE `language_id`=' . $id . ' AND `type`=' . $type . ' AND subcategory_id='.$cat_id.' ORDER BY row_order + 0 ASC';
    $db->sql($sql);
    $res = $db->getResult();
    ...
    if (isset($_POST['sortable']) && $_POST['sortable'] == 'sortable') {
        ...
        $options = '<option value="">Select Sub 2 Category</option>';
        foreach ($res as $option) {
            $options .= "<option value='" . $option['id'] . "'>" . $option['sub_2category_name'] . "</option>";
        }
    }
    echo $options;
}
//end
//get_sub_2category_of_subcategories
if (isset($_POST['get_sub_2categories_of_subcategory']) && $_POST['get_sub_2categories_of_subcategory'] != '') {
    $id = $_POST['language_id'];
    $cat_id = $_POST['subcategory_id'];
    // $type = (isset($_POST['type'])) ? $_POST['type'] : 1;
    $type = $_POST['type'];
    if (empty($id)) {
        echo '<option value="">Select Sub 2 Category</option>';
...
//add_daily_news
if (isset($_POST['language_id']) && isset($_POST['adddailynews'])) {
    $type = $db->escapeString($_POST['type']);
    $title = $db->escapeString($_POST['title']);
    $maincat_id = $db->escapeString($_POST['maincat_id']);
    $description = $db->escapeString($_POST['detail']);
    $language_id = ($fn->is_language_mode_enabled()) ? $db->escapeString($_POST['language_id']) : 0;
      $filename = '';
    // common image file extensions
    if ($_FILES['image']['error'] == 0 && $_FILES['image']['size'] > 0) {
        if (!is_dir('images/dailynews')) {
            mkdir('images/dailynews', 0777, true);
        }
    ...
    $sql = "INSERT INTO `dailynews` (`language_id`, `maincat_id`, `type`, `title`, `description`,`image`) VALUES ('" . $language_id . "','" . $maincat_id . "','" . $type . "','" . $title . "','".$description."','".$filename."')";
    $db->sql($sql);

    echo '<label class="alert alert-success">Daily News created successfully!</label>';
}
//end
//update_dailynews
if(isset($_POST['update_dailynews']) && isset($_POST['dailynews_id'])){
    $id = $db->escapeString($_POST['dailynews_id']);
    $title = $db->escapeString($_POST['title']);
    $detail = $db->escapeString($_POST['detail']);
    $maincat_id = $db->escapeString($_POST['maincat_id']);
    $langauge_id = $db->escapeString($_POST['language']);
    $type = $db->escapeString($_POST['type']);
    $sql = "UPDATE dailynews SET title='$title', description='$detail',language_id='$langauge_id',maincat_id='$maincat_id' WHERE id='$id' AND type='$type'";
    if($db->sql($sql)){
        echo "<script> alert('Daily News Updated Successfully.');location.href='daily-news.php';</script>";
        // echo $language_id;
    }else{
        echo "<script> alert('Daily News Not Updated Successfully.');location.href='daily-news.php';</script>";
    }
}
//end
//add_banner_image
if (isset($_POST['add_banner_image']) && ($_POST['add_banner_image'])==1) {
    // $type = $db->escapeString($_POST['type']);
    $title = $db->escapeString($_POST['title']);
    $type = 9;
      $filename = '';
    // common image file extensions
    if ($_FILES['image']['error'] == 0 && $_FILES['image']['size'] > 0) {
        if (!is_dir('images/banner_image')) {
            mkdir('images/banner_image', 0777, true);
        }
    ...
    $sql = "INSERT INTO `banner_image` (`link`,`image`,`type`) VALUES ('" . $title . "','".$filename."','".$type."')";
    $db->sql($sql);

    echo '<label class="alert alert-success">Banner Image Added successfully!</label>';
}
//end
//addcurrentaffair
if (isset($_POST['language_id']) && isset($_POST['addcurrentaffair'])) {
    $type = $db->escapeString($_POST['type']);
    $title = $db->escapeString($_POST['title']);
    $maincat_id = $db->escapeString($_POST['maincat_id']);
    ...
    $sql = "INSERT INTO `currentaffair` (`language_id`, `maincat_id`, `type`, `title`, `description`,`subcategory_id`) VALUES ('" . $language_id . "','" . $maincat_id . "','" . $type . "','" . $title . "','".$description."','".$subcategory_id."')";
    $db->sql($sql);

    echo '<label class="alert alert-success">Current Affair created successfully!</label>';
}
//end
//update_current_affair
if(isset($_POST['update_currentaffair']) && isset($_POST['dailynews_id'])){
    $id = $db->escapeString($_POST['dailynews_id']);
    $title = $db->escapeString($_POST['title']);
    $detail = $db->escapeString($_POST['detail']);
    $maincat_id = $db->escapeString($_POST['maincat_id']);
    ...
    $sql = "UPDATE currentaffair SET title='$title', description='$detail',language_id='$langauge_id',maincat_id='$maincat_id',subcategory_id='$subcat_id' WHERE id='$id' AND type='$type'";
    if($db->sql($sql)){
        echo "<script> alert('Current Affair Updated Successfully.');location.href='currentaffair.php';</script>";
        // echo $language_id;
    }else{
        echo "<script> alert('Current Affair Not Updated Successfully.');location.href='currentaffair.php';</script>";
    }
}
//end
//2. update_category
...
if (isset($_GET['delete_category']) && $_GET['delete_category'] != '') {
    ...
    if ($db->sql($sql)) {
        ...
        $sql3 = 'DELETE FROM `tbl_learning` WHERE `category`=' . $id;
        $db->sql($sql3);
...
//delete_banner_image
if (isset($_GET['delete_banner_image']) && $_GET['delete_banner_image'] != '') {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
...
if (isset($_POST['name']) && isset($_POST['add_subcategory'])) {
    $name = $db->escapeString($_POST['name']);
    $type = $db->escapeString($_POST['add_subcategory']);
    $maincat_id = $db->escapeString($_POST['maincat_id']);
    $language_id = ($fn->is_language_mode_enabled()) ? $db->escapeString($_POST['language_id']) : 0;
    ...
    $sql = "INSERT INTO `subcategory` (`language_id`,`maincat_id`,`subcategory_name`,`type`, `image`,`row_order`) VALUES ('" . $language_id . "','" . $maincat_id . "','" . $name . "','" . $type . "','" . $filename . "','0')";
    $db->sql($sql);
...
if (isset($_POST['name']) && isset($_POST['add_subcategory'])) {
    ...
    $sql = "INSERT INTO `subcategory` (`language_id`,`maincat_id`,`subcategory_name`,`type`, `image`,`row_order`) VALUES ('" . $language_id . "','" . $maincat_id . "','" . $name . "','" . $type . "','" . $filename . "','".$status."','0')";
    $db->sql($sql);

    echo '<label class="alert alert-success">Q/A Sub Category created successfully!</label>';
}
//add_sub_2category
if (isset($_POST['name']) && isset($_POST['add_subcategory2'])) {
    $name = $db->escapeString($_POST['name']);
    $status=1;
    $type = $db->escapeString($_POST['add_subcategory2']);
    $maincat_id = $db->escapeString($_POST['subcat_id']);
    $language_id = ($fn->is_language_mode_enabled()) ? $db->escapeString($_POST['language_id']) : 0;

    $filename = '';
    // common image file extensions
    if ($_FILES['image']['error'] == 0 && $_FILES['image']['size'] > 0) {
        if (!is_dir('images/subcategory_2')) {
            mkdir('images/subcategory_2', 0777, true);
        }
        $extension = pathinfo($_FILES["image"]["name"])['extension'];
        if (!(in_array($extension, $allowedExts))) {
            $response['error'] = true;
        ...
        $target_path = 'images/subcategory_2/';
        $filename = microtime(true) . '.' . strtolower($extension);
        $full_path = $target_path . "" . $filename;
        if (!move_uploaded_file($_FILES["image"]["tmp_name"], $full_path)) {
            $response['error'] = true;
    ...
    $sql = "INSERT INTO `sub_2category` (`language_id`,`subcategory_id`,`sub_2category_name`, `image`,`status`,`row_order`,`type`) VALUES ('" . $language_id . "','" . $maincat_id . "','" . $name . "','" . $filename . "','".$status."','0','" . $type . "')";
    $db->sql($sql);

    echo '<label class="alert alert-success">Sub Category 2 created successfully!</label>';
}

//end
//add_subcategory_3
if (isset($_POST['name']) && isset($_POST['add_subcategory3'])) {
    $name = $db->escapeString($_POST['name']);
    $status=1;
    $type = $db->escapeString($_POST['add_subcategory3']);
    $maincat_id = $db->escapeString($_POST['sub_2category_id']);
    $language_id = ($fn->is_language_mode_enabled()) ? $db->escapeString($_POST['language_id']) : 0;

    $filename = '';
    // common image file extensions
    if ($_FILES['image']['error'] == 0 && $_FILES['image']['size'] > 0) {
        if (!is_dir('images/subcategory_3')) {
            mkdir('images/subcategory_3', 0777, true);
        }
        $extension = pathinfo($_FILES["image"]["name"])['extension'];
        if (!(in_array($extension, $allowedExts))) {
            $response['error'] = true;
        ...
        $target_path = 'images/subcategory_3/';
        $filename = microtime(true) . '.' . strtolower($extension);
        $full_path = $target_path . "" . $filename;
        if (!move_uploaded_file($_FILES["image"]["tmp_name"], $full_path)) {
            $response['error'] = true;
    ...
    $sql = "INSERT INTO `sub_3category` (`language_id`,`sub_2category_id`,`sub_3category_name`, `image`,`status`,`row_order`,`type`) VALUES ('" . $language_id . "','" . $maincat_id . "','" . $name . "','" . $filename . "','".$status."','0','" . $type . "')";
    $db->sql($sql);

    echo '<label class="alert alert-success">Sub Category 3 created successfully!</label>';
}
//end
//update_ebook
if (isset($_POST['ebook_id']) && isset($_POST['update_ebook'])) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    $id = $_POST['ebook_id'];
    $title = $db->escapeString($_POST['title']);
    $category_id = $db->escapeString($_POST['category']);
    $sub_category_id = $db->escapeString($_POST['subcategory']);
    $sub_2category_id = $db->escapeString($_POST['sub_2category']);
    $language_id=$db->escapeString($_POST['language_id']);
    
    $sql = "Update ebook set `language_id`='" . $language_id . "', `category_id`='" . $category_id . "', `subcategory_id`='".$sub_category_id."',`sub_2category_id`='".$sub_2category_id."',`title`='".$title."' where id='".$id."' ";
    
    $db->sql($sql);

    echo "<p class='alert alert-success'>Ebook updated successfully!</p>";
}
//end
//5. update_subcategory
...
//update_sub_2category
if (isset($_POST['update_sub_2category']) && isset($_POST['update_sub_2category'])) {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    $id = $_POST['update_sub_2category_id'];
    $name = $db->escapeString($_POST['update_sub_2cat']);
    $maincat_id = $db->escapeString($_POST['update_subcategory_id']);
    
    $language_id = ($fn->is_language_mode_enabled()) ? $db->escapeString($_POST['language_id']) : 0;

    $status = $db->escapeString($_POST['update_status']);
    if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0) {
        if (!is_dir('images/sub_2category')) {
            mkdir('images/sub_2category', 0777, true);
        }
        //image isn't empty and update the image
        $image_url = $db->escapeString($_POST['image_url']);
        ...
        $target_path = 'images/sub_2category/';
        $filename = microtime(true) . '.' . strtolower($extension);
        $full_path = $target_path . "" . $filename;
        if (!move_uploaded_file($_FILES["image"]["tmp_name"], $full_path)) {
            echo '<p class="alert alert-danger">Image type is invalid</p>';
        ...
        $sql = "Update sub_2category set `image`='" . $filename . "' where `id`=" . $id;
        $db->sql($sql);
    }

    $sql = "Update sub_2category set `subcategory_id`='" . $maincat_id . "', `sub_2category_name`='" . $name . "', `status`='" . $status . "' ";
    $sql .= ($fn->is_language_mode_enabled()) ? ", `language_id` = " . $language_id . " " : "";
    $sql .= " where `id`=" . $id;
    $db->sql($sql);

    $sql = "Update question set `sub_2category`='" . $maincat_id . "' ";
    $sql .= ($fn->is_language_mode_enabled()) ? ", `language_id` = " . $language_id . " " : "";
    $sql .= " where `sub_2category` =" . $id;
    $db->sql($sql);

    echo "<p class='alert alert-success'>Sub category updated successfully!</p>";
}
//delete_sub_new2category
if(isset($_GET['delete_sub_new2category']) && $_GET['delete_sub_new2category']!=''){
    if(!checkadmin($auth_username)){
        echo "<label class='alert alert-danger'>Access denied -You are not authorized to access this page.</label>";
        return false;
    }
...
//delet_ebook
if(isset($_GET['delete_ebook']) && $_GET['delete_ebook']!=''){
    if(!checkadmin($auth_username)){
        echo "<label class='alert alert-danger'>Access denied -You are not authorized to access this page.</label>";
        return false;
    }
    $id = $_GET['id'];
    $type=$_GET['type'];
    $sql = 'DELETE FROM `ebook` WHERE id='.$id.' AND type='.$type;
    if($db->sql($sql)){
        echo 1;
    }else{
        echo $sql;
...
//delete_sub_2category
if (isset($_GET['delete_sub_2category']) && $_GET['delete_sub_2category'] != '') {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
...
//end
//delete_daily_News
if (isset($_GET['delete_dailynews']) && $_GET['delete_dailynews'] != '') {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    ...
    $image = $_GET['image'];
    $type = $_GET['type'];
    $sql = 'DELETE FROM `dailynews` WHERE `id`=' . $id.' AND type='.$type;
    if ($db->sql($sql)) {
        echo 1;
    } else {
        echo 0;
...
//end
//delete_current_affair
if (isset($_GET['delete_currentaffair']) && $_GET['delete_currentaffair'] != '') {
    if (!checkadmin($auth_username)) {
        echo "<label class='alert alert-danger'>Access denied - You are not authorized to access this page.</label>";
        return false;
    }
    ...
    $image = $_GET['image'];
    $type = $_GET['type'];
    $sql = 'DELETE FROM `currentaffair` WHERE `id`=' . $id.' AND type='.$type;
    if ($db->sql($sql)) {
        echo 1;
    } else {
        echo 0;
...
if (isset($_POST['question']) && isset($_POST['add_question'])) {
    ...
    $type = $db->escapeString($_POST['add_question']);
    $subcategory = (empty($_POST['subcategory'])) ? 0 : $db->escapeString($_POST['subcategory']);
    $sub_2category = (empty($_POST['sub_2category'])) ? 0 : $db->escapeString($_POST['sub_2category']);
    $sub_3category = (empty($_POST['sub_3category'])) ? 0 : $db->escapeString($_POST['sub_3category']);
    ...
    $sql = "INSERT INTO `question`(`category`, `subcategory`,`sub_2category_id`,`sub_3category_id`, `language_id`, `image`, `question`, `question_type`, `optiona`, `optionb`, `optionc`, `optiond`, `answer`,`type`) VALUES 
	('" . $category . "','" . $subcategory . "','" . $sub_2category . "','" . $sub_3category . "','" . $language_id . "','" . $filename . "','" . $question . "','" . $question_type . "','" . $a . "','" . $b . "','" . $c . "','" . $d . "','" . $answer . "','".$type."')";
...
if (isset($_GET['delete_multiple']) && $_GET['delete_multiple'] != '') {
    ...
    $sql = "DELETE FROM `" . $table . "` WHERE `id` in ( " . $ids . " ) AND type=".$type;
    if ($db->sql($sql)) {
        echo 1;
    } else {
        echo 0;
...
if (isset($_GET['delete_contest']) && $_GET['delete_contest'] != '') {
    ...
    if ($db->sql($sql)) {
        /* delete questions */
        $sql = 'DELETE FROM `contest_questions` WHERE `contest_id`=' . $id;
        $db->sql($sql);
...
if (isset($_POST['title']) && isset($_POST['add_learning'])) {
    ...
    $title = $db->escapeString($_POST['title']);
    $detail = $db->escapeString($_POST['detail']);
    $subcategory_id=$db->escapeString($_POST['subcategory']);
    $sub_2category_id=$db->escapeString($_POST['sub_2category']);
    $sub_3category_id=$db->escapeString($_POST['sub_3category']);
    $status=$db->escapeString($_POST['status']);
    $type=$db->escapeString($_POST['type']);

    $sql = "INSERT INTO `tbl_learning` ( `category`, `language_id`,`subcategory_id`,`sub_2category_id`,`sub_3category_id`, `title`, `detail`, `status`,`type`) VALUES ('" . $category . "','" . $language_id . "','" . $subcategory_id . "','" . $sub_2category_id . "','" . $sub_3category_id . "','" . $title . "','" . $detail . "','".$status."','".$type."')";
    $db->sql($sql);
    $res = $db->getResult();
    echo '<label class="alert alert-success">Question/Answer created successfully!</label>';
}
